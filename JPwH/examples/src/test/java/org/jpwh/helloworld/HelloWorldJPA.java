...
public class HelloWorldJPA extends TransactionManagerTest {
    ...
    public void storeLoadMessage() throws Exception {
        ...
        {
            {
                // Get access to the standard transaction API <code>UserTransaction</code> and begin a transaction on this thread of execution.
                UserTransaction tx = _TM.getUserTransaction();
                tx.begin();

                // Begin a new session with the database by creating an <code>EntityManager</code>, this is your context for all persistence operations.
                EntityManager em = emf.createEntityManager();

                // Create a new instance of the mapped domain model class <code>Message</code> and set its <code>text</code> property.
                 Message message = new Message();
                 message.setText("Hello World!");

                // Enlist the transient instance with your persistence context, you make it persistent.
                // Hibernate now knows that you wish to store that data, it doesn't necessarily call the database immediately, however.
                em.persist(message);

                // Commit the transaction, Hibernate now automatically checks the persistence context and executes the necessary SQL <code>INSERT</code> statement.
                tx.commit();
